...
fn main() {
    ...
    let new_block = Block::new(
        3,
        new_BC.blocks[1].hash.to_owned(),
        "Ivan ponders GREEN. Decides to reject GREEN. very tough call".to_string(),
    );
    new_BC.try_add_block(new_block);

    let new_block = Block::new(
        4,
        new_BC.blocks[2].hash.to_owned(),
        "Ivan joins BLUE".to_string(),
    );
    new_BC.try_add_block(new_block);
